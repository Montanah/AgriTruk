  ...
  try {
    ...
    if (typeof dateString === "object" && dateString !== null) {
      if ("toDate" in dateString && typeof dateString.toDate === "function") {
        ...
        date = new Date(
          dateString.seconds * 1000 + (dateString.nanoseconds || 0) / 1000000,
        );
...
const DisputeDetailScreen = () => {
  ...
  const params =
    (route.params as { disputeId?: string; action?: string }) || {};
  ...
  useEffect(() => {
    console.log("🔍 [DisputeDetail] useEffect triggered:", {
      disputeId,
      params,
    });
    if (disputeId) {
      ...
      console.error(
        "❌ [DisputeDetail] No disputeId provided in route params. Route params:",
        route.params,
      );
  ...
  const loadDispute = async () => {
    if (!disputeId) {
      console.error(
        "DisputeDetailScreen: Cannot load dispute without disputeId",
      );
      return;
    }
    ...
    try {
      ...
      if (data.booking) {
        ...
        console.warn(
          "⚠️ [DisputeDetail] Booking not populated by service, fetching manually...",
        );
        try {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "open":
        return colors.warning; // Backend uses 'open' not 'pending'
      case "resolved":
        return colors.success;
      case "in_progress":
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.emptyContainer}>
          <MaterialCommunityIcons
            ...
            color={colors.error}
          />
          <Text style={styles.emptyText}>{error || "Dispute not found"}</Text>
          {error && (
            ...
            <View style={styles.statusHeader}>
              <View style={styles.disputeIdRow}>
                <View style={styles.disputeIdContainer}>
                  ...
                  />
                  <Text
                    style={styles.disputeId}
                    numberOfLines={1}
                    ellipsizeMode="tail"
                  >
                    {dispute.disputeId ||
                      `DSP${(dispute.id || dispute.bookingId || "").substring(0, 6).toUpperCase()}`}
                  </Text>
                </View>
              </View>
              <View style={styles.badgeContainer}>
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        getPriorityColor(dispute.priority) + "15",
                      borderColor: getPriorityColor(dispute.priority) + "40",
                    },
                  ]}
                >
                  <View
                ...
                <View
                  style={[
                    styles.statusBadge,
                    {
                      ...
                      borderColor: getStatusColor(dispute.status) + "40",
                    },
                  ]}
                >
                  <View
        ...
        {(dispute.transporter ||
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <MaterialCommunityIcons
                name={
                  dispute.transporterType === "driver"
                    ? "account-circle"
                    : "truck-delivery"
                }
                size={20}
            ...
            {(() => {
              const driver =
                ...
                dispute.transporter;
              const transporter =
                bookingDetails?.transporter || dispute.transporter;
              const company =
                driver?.company ||
                transporter?.company ||
              ...
              {(() => {
                ...
                const complainantId =
                  typeof dispute.openedBy === "object"
                    ? dispute.openedBy.id || dispute.openedBy.uid
                    : dispute.openedBy;
                const accusedId =
                  dispute.transporterId ||
              ...
              {dispute.amountRefunded && dispute.amountRefunded > 0 && (
                <View style={styles.amountRow}>
                  ...
                  <Text style={styles.resolutionAmount}>
                    Amount Refunded: KES{" "}
                    {dispute.amountRefunded.toLocaleString()}
                  </Text>
                </View>
              )}
              {dispute.resolvedAt && (
        ...
        <View style={styles.actionsContainer}>
          ...
          >
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate(
                  "CreateDispute" as never,
                  { disputeId: dispute.disputeId, edit: true } as never,
                )
              }
            >
              <MaterialCommunityIcons
      ...
      {dispute &&
        (() => {
          ...
          const complainantName =
            typeof dispute.openedBy === "object" && dispute.openedBy !== null
              ? dispute.openedBy.name ||
                dispute.openedBy.displayName ||
                dispute.customer?.name ||
                "Customer"
              : "Customer";
          const complainantPhoto =
            typeof dispute.openedBy === "object" && dispute.openedBy !== null
              ? dispute.openedBy.profilePhoto ||
                dispute.openedBy.photoURL ||
                dispute.customer?.profilePhoto
              : undefined;
          ...
          if (bookingDetails?.transporterId) {
            ...
            console.log(
              "✅ [DisputeDetail] Using userId from booking.transporterId:",
              accusedId,
            );
          }
          // Priority 2: Get userId from dispute.transporter object
            ...
            if (accusedId) {
              console.log(
                "✅ [DisputeDetail] Using userId from dispute.transporter:",
                accusedId,
              );
            }
          }
          ...
          // If we have bookingDetails with assignedDriver, try to get userId from there
            ...
            console.log(
              "✅ [DisputeDetail] Using userId from bookingDetails.assignedDriver.userId:",
              accusedId,
            );
          }
          // Priority 4: Fallback to dispute.transporterId (might be driver ID - will fail chat but we'll log it)
            ...
            console.warn(
              "⚠️ [DisputeDetail] Using dispute.transporterId as fallback (may be driver ID, not userId):",
              accusedId,
            );
            console.warn(
              '⚠️ [DisputeDetail] This may cause "Participant not found" error if it\'s a driver ID',
            );
          }
          ...
          if (!participant1Id || !participant2Id) {
            console.error(
              "❌ [DisputeDetail] Cannot create chat: missing participant IDs",
              {
                participant1Id,
                participant2Id,
                complainantId,
          ...
          return (
            <RealtimeChatModal
              ...
              onChatCreated={(chatRoom) => {
                console.log(
                  "✅ [DisputeDetail] Dispute chat room created:",
                  chatRoom,
                );
              }}
            />
