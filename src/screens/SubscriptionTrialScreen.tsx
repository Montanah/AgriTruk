...
const SubscriptionTrialScreen: React.FC<SubscriptionTrialScreenProps> = ({
  route,
}) => {
  const navigation = useNavigation<NavigationProp>();
  const { userType, transporterType, subscriptionStatus, isRenewal } =
    route.params;
  ...
  const handleCardSubmit = async (cardData: CardData) => {
    if (!isCardValid) {
      Alert.alert(
        "Invalid Card",
        "Please fix the card errors before proceeding.",
      );
      return;
    }
    ...
    try {
        ...
        if (userType === "company") {
          ...
          try {
            const subscriptionStatus =
              await subscriptionService.startCompanyFleetTrial(user.uid);
            result = {
              success: true,
            ...
            console.error(
              "Company fleet trial failed, trying regular trial:",
              error,
            );
            result = await subscriptionService.activateTrial("transporter");
          }
        } else {
          result = await subscriptionService.activateTrial(
            userType as "individual" | "broker",
          );
        }
      }
      ...
      if (result.success) {
        if (result.existingSubscription) {
          ...
          try {
            ...
            const user = auth.currentUser;
            if (user) {
              await NotificationHelper.sendSubscriptionNotification(
                "activated",
                {
                  userId: user.uid,
                  role: userType,
            ...
            console.warn(
              "Failed to send trial activation notification:",
              notificationError,
            );
          }
          ...
          setTimeout(() => {
            try {
              ...
              console.warn(
                "Automatic navigation failed, user can use manual button:",
                navError,
              );
      ...
      Alert.alert(
        ...
        [
          {
            text: "Retry",
            onPress: () => activateTrial(paymentMethod, paymentData),
          },
    ...
    try {
      ...
      try {
        if (
          ...
          (navigation as any).navigate("TransporterTabs", {
              ...
              <View style={styles.formCard}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!mpesaPhone.trim() || mpesaPhone.trim().length < 10) &&
              ...
              {!isForRenewal && (
                <>
                  ...
                  <View style={styles.benefitItem}>
                    ...
                    />
                    <Text style={styles.benefitText}>
                      $1 test charge will be refunded immediately
                    </Text>
                  </View>
                </>
              )}
