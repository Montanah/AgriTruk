...
export default function ManageTransporterScreen({ route }: any) {
  ...
  const handleActivateTrial = async () => {
    try {
      ...
      if (result.success) {
        ...
        Alert.alert(
          "Activation Failed",
          result.message || "Failed to activate trial.",
        );
      }
    } catch (error) {
  ...
  const [loadingIndividualProfile, setLoadingIndividualProfile] =
    useState(true);
  const [individualProfilePhoto, setIndividualProfilePhoto] =
    useState<any>(null);
  ...
  const currentLocationForDisplay =
    currentLocation || individualProfile?.lastKnownLocation;

  // Modal states
  const [insuranceModalVisible, setInsuranceModalVisible] = useState(false);
  const [licenseModalVisible, setLicenseModalVisible] = useState(false);
  const [photoGalleryModalVisible, setPhotoGalleryModalVisible] =
    useState(false);
    ...
    const fetchData = async () => {
      try {
        ...
        if (transporterType === "company" && companyProfile?.companyId) {
          ...
          const driversData = await apiRequest(
            `/companies/${companyProfile.companyId}/drivers`,
          );
          // Only show active drivers for companies
          const activeDrivers =
            driversData?.drivers?.filter((d) => d.status === "active") || [];
          ...
          const vehiclesData = await apiRequest(
            `/companies/${companyProfile.companyId}/vehicles`,
          );
          // Only show approved vehicles for companies
          const approvedVehicles =
            vehiclesData?.vehicles?.filter((v) => v.status === "approved") ||
            [];
        ...
        if (
          ...
          console.log(
            "Company has no fleet data yet - this is normal for new companies",
          );
        }
      } finally {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            "🔍 User profile data from /auth/profile:",
            userProfileData,
          );
          console.log(
            "📧 emailVerified from user collection:",
            userProfileData.emailVerified,
          );
          console.log(
            "📱 phoneVerified from user collection:",
            userProfileData.phoneVerified,
          );
          console.log(
            "✅ isVerified from user collection:",
      ...
      const fetchProfile = async () => {
        try {
          ...
          if (transporterRes.ok) {
            ...
            setAcceptingBooking(
              transporterData.transporter?.acceptingBooking || false,
            );
            if (transporterData.transporter?.driverProfileImage) {
              setIndividualProfilePhoto({
                uri: transporterData.transporter.driverProfileImage,
              });
            }
          }
          ...
          if (userRes.ok) {
            ...
            console.log(
              "🔍 Individual transporter user profile data:",
              userProfileData,
            );
            console.log(
              "📧 emailVerified from user collection (individual):",
              userProfileData.emailVerified,
            );
            console.log(
              "📱 phoneVerified from user collection (individual):",
              userProfileData.phoneVerified,
            );
            console.log(
              "✅ isVerified from user collection (individual):",
              userProfileData.isVerified,
            );
          ...
          console.error(
            "Error fetching individual transporter profile:",
            error,
          );
        }
    ...
    try {
      ...
      if (transporterType === "individual") {
        ...
        if (response.ok) {
          ...
          Alert.alert(
            "Error",
            "Failed to update vehicle body type. Please try again.",
          );
        }
      } else {
      ...
      const setupLocationTracking = async () => {
        try {
          ...
          if (!hasConsent) {
            ...
            console.log(
              "⚠️ ManageTransporterScreen: No background location consent found - showing disclosure modal",
            );
    ...
    Alert.alert(
      ...
      [
        { text: "Take Photo", onPress: () => pickProfilePhotoCamera() },
        {
          ...
          onPress: () => pickProfilePhotoGallery(),
        },
      ...
      if (
        ...
        try {
          ...
          if (user && companyProfile?.companyId) {
            ...
            console.log(
              "Uploading company logo to:",
              `${API_ENDPOINTS.COMPANIES}/${companyProfile.companyId}/upload`,
            );
          ...
          try {
            ...
            if (user) {
              ...
              if (editRegistrationNumber && editRegistrationNumber.trim()) {
                companyUpdatePayload.registration =
                  editRegistrationNumber.trim();
              }

              // Update company profile if there are changes
              if (Object.keys(companyUpdatePayload).length > 0) {
                const companyResponse = await fetch(
                  `${API_ENDPOINTS.COMPANIES}/${companyProfile.companyId}`,
                  {
                    method: "PATCH",
                    headers: {
                      Authorization: `Bearer ${token}`,
            ...
            console.error(
              "Error updating company registration:",
              companyUpdateError,
            );
        ...
        setUserProfile((prev: any) =>
          prev
            ? {
                ...
                phoneNumber: editPhone || prev.phoneNumber,
              }
            : prev,
        );
  ...
  const [assignModalVehicleIdx, setAssignModalVehicleIdx] = useState<
    number | null
  >(null);
  ...
  const handleRecruitDriver = async () => {
    ...
    try {
      ...
      const companyResponse = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL || "https://agritruk.onrender.com"}/api/companies/transporter/${user.uid}`,
        {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL || "https://agritruk.onrender.com"}/api/companies/${company?.id}/drivers`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        Alert.alert(
          "Success",
          "Driver recruited successfully! They will receive login credentials via email.",
        );
  ...
  const pickDriverPhoto = async (setFn: (value: any) => void) => {
    Alert.alert(
      ...
      [
        ...
        {
          text: "Choose from Gallery",
          onPress: () => pickDriverPhotoFromGallery(setFn),
        },
        { text: "Cancel", style: "cancel" },
      ],
    );
  ...
  const pickDriverIdDoc = async (setFn: (value: any) => void) => {
    Alert.alert(
      ...
      [
        ...
        {
          text: "Choose from Gallery",
          onPress: () => pickDriverIdDocFromGallery(setFn),
        },
  ...
  const pickDriverLicense = async (setFn: (value: any) => void) => {
    Alert.alert(
      ...
      [
        {
          text: "Take Photo",
          onPress: () => pickDriverLicenseFromCamera(setFn),
        },
        {
          text: "Choose from Gallery",
          onPress: () => pickDriverLicenseFromGallery(setFn),
        },
  ...
  const openEditVehicle = (idx: number) => {
    ...
    setVehicleReg(v.reg);
    setRefrigeration(v.refrigeration);
    setHumidityControl(v.humidityControl);
    setSpecialCargo(v.specialCargo);
    setVehicleFeatures(v.features);
    setInsurance(v.insurance);
    setVehiclePhotos(v.photos);
    setAssignedDriverId(v.assignedDriverId || null);
    setVehicleModal(true);
  };
  const handleSaveVehicle = async () => {
    ...
    if (
      ...
      validationErrors.push(
        "Vehicle Year must be between 1990 and " +
    ...
    if (!companyProfile?.id && !companyProfile?.companyId) {
      Alert.alert(
        "Error",
        "Company profile not loaded. Please refresh and try again.",
      );
    ...
    try {
      ...
      if (!companyId) {
        throw new Error(
          "Company ID not found in profile. Please refresh and try again.",
        );
      }
      ...
      if (vehiclePhotos.length > 0) {
        vehiclePhotos.forEach((photo, index) => {
          formData.append("vehicleImages", {
            ...
            name:
              photo.fileName || photo.name || `vehicle_photo_${index + 1}.jpg`,
          } as any);
        });
      }
  ...
  const handleSaveDriver = () => {
    if (!driverName || !driverPhoto || !driverLicense) {
      Alert.alert(
        "Missing Info",
        "Please provide name, profile photo, and license.",
      );
      return;
    }
    const driver = {
  ...
  const handleRemoveDriver = (idx: number) => {
    ...
    setVehicles(
      vehicles.map((v) =>
        v.assignedDriverId === drivers[idx].id
  ...
  const handleVerifyEmail = async () => {
    const emailToVerify =
      userProfile?.email || editEmail || auth.currentUser?.email;
    if (!emailToVerify) {
      Alert.alert(
        "Error",
  ...
  const handleVerifyPhone = async () => {
    const phoneToVerify =
      userProfile?.phone || editPhone || auth.currentUser?.phoneNumber;
    if (!phoneToVerify) {
      Alert.alert(
        "Error",
  ...
  const assignDriverToVehicle = async (
    ...
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINTS.VEHICLES}/${vehicle.id}/assign-driver`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Failed to assign driver to vehicle: ${response.status}`,
        );
      }
      ...
      setVehicles(
        vehicles.map((v, i) =>
          i === vehicleIdx ? { ...v, assignedDriverId: driverId } : v,
        ),
      );
  ...
  const unassignDriverFromVehicle = async (vehicleIdx: number) => {
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINTS.VEHICLES}/${vehicle.id}/unassign-driver`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Failed to unassign driver from vehicle: ${response.status}`,
        );
      }
      ...
      setVehicles(
        vehicles.map((v, i) =>
          i === vehicleIdx ? { ...v, assignedDriverId: null } : v,
        ),
      );
      ...
      Alert.alert(
        "Unassignment Failed",
        error?.message ||
          "Failed to unassign driver from vehicle. Please try again.",
        [{ text: "OK" }],
      );
    }
  };
      ...
      <>
        <FormKeyboardWrapper
          ...
          contentContainerStyle={[
            styles.container,
            { paddingTop: 32, paddingBottom: 100 },
          ]}
        >
          ...
          <View style={styles.headerContainer}>
            ...
            >
              Manage Vehicles, Drivers, Assignments
            </Text>
            <TouchableOpacity
          ...
          <View style={styles.card}>
              ...
              <TouchableOpacity
                onPress={() => {
                  ...
                  setEditRegistrationNumber(
                    companyProfile?.companyRegistration ||
            ...
            >
              {editProfilePhoto ? (
                <Image
                  source={{ uri: editProfilePhoto.uri }}
                  style={{
                    width: 80,
                    height: 80,
                    borderRadius: 12,
                ...
                <Image
                  source={{ uri: companyProfile.companyLogo }}
                  style={{
                    width: 80,
                    height: 80,
                    borderRadius: 12,
            ...
            <View style={styles.companyInfo}>
              ...
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoText}>
                  {companyProfile?.companyRegistration ||
                    "Registration Number (Optional - Required after 5 trips)"}
                </Text>
              </View>
              <View style={styles.infoRow}>
                <MaterialCommunityIcons
                ...
                />
                <Text style={styles.infoText}>
                  {companyProfile?.companyContact || "Contact Number"}
                </Text>
              </View>
              <View style={styles.infoRow}>
                <MaterialCommunityIcons
                ...
                />
                <Text style={styles.infoText}>
                  {companyProfile?.companyEmail || "Email Address"}
                </Text>
              </View>
              <View style={styles.infoRow}>
                <MaterialCommunityIcons
                ...
                <Text style={styles.infoText}>
                  ...
                  companyProfile.companyAddress.trim() !== ""
                    ? companyProfile.companyAddress
                    : "No address provided"}
                </Text>
              </View>
            ...
            >
              ...
              >
                Registration number is required after 5 completed trips. Update
                via Edit Profile above.
              </Text>
            </View>
          ...
          <View style={styles.card}>
            ...
            {((userProfile?.email && isGeneratedEmail(userProfile.email)) ||
              (userProfile?.phone && isGeneratedPhone(userProfile.phone))) && (
              <View
                style={{
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      isGeneratedPhone(userProfile?.phone)
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              >
                ...
                >
                  <MaterialCommunityIcons
                    ...
                    size={12}
                    color={
                      userProfile?.emailVerified ? colors.success : colors.error
                    }
                  />
                  <Text
                    style={[
              ...
              {isGeneratedEmail(userProfile?.email || editEmail) && (
                ...
                >
                  This email is auto-generated. Please update it before
                  verifying.
                </Text>
              )}
              {!userProfile?.emailVerified && (
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              >
                ...
                >
                  <MaterialCommunityIcons
                    ...
                    size={12}
                    color={
                      userProfile?.phoneVerified ? colors.success : colors.error
                    }
                  />
                  <Text
                    style={[
              ...
              {isGeneratedPhone(userProfile?.phone || editPhone) && (
                ...
                >
                  This phone number is auto-generated. Please update it before
                  verifying.
                </Text>
              )}
              {!userProfile?.phoneVerified && (
          ...
          <View style={styles.card}>
            <UnifiedSubscriptionCard
              ...
              userType={
                transporterType === "company" ? "company" : "transporter"
              }
              onManagePress={() => {
                  ...
                  <View style={styles.vehicleListItem} key={item.id}>
                    ...
                    <Text style={styles.value}>
                      ...
                      {item.assignedDriverId ? (
                        ...
                        >
                          {drivers.find((d) => d.id === item.assignedDriverId)
                            ?.name || "Unknown"}
                        </Text>
                      ) : (
                        <Text
                    ...
                    <View
                      style={{ marginTop: 6, flexDirection: "row", gap: 8 }}
                    >
                      <TouchableOpacity
                        style={[
                          styles.driverAssignBtn,
                          { minWidth: 120, justifyContent: "center" },
                        ]}
                        onPress={() => {
                          setAssignModalVehicleIdx(index);
                            ...
                            Alert.alert(
                              ...
                              [
                                { text: "Cancel", style: "cancel" },
                                {
                                  ...
                                  style: "destructive",
                                  onPress: () =>
                                    unassignDriverFromVehicle(index),
                                },
                              ],
                            );
                          }}
          ...
          >
            <View style={styles.modalOverlay}>
              <View style={styles.vehicleModalCard}>
                ...
                {assignModalVehicleIdx !== null &&
                  vehicles[assignModalVehicleIdx] && (
                    <View
                      style={{
                        backgroundColor: colors.background.secondary,
                        padding: 12,
                    ...
                    >
                      ...
                      >
                        {vehicles[assignModalVehicleIdx].make || "Unknown"}{" "}
                        {vehicles[assignModalVehicleIdx].model || ""} (
                        {vehicles[assignModalVehicleIdx].year || "N/A"})
                      </Text>
                      <Text
                        style={{
                          fontSize: 14,
                      ...
                      >
                        Registration:{" "}
                        {vehicles[assignModalVehicleIdx].reg ||
                          vehicles[assignModalVehicleIdx].vehicleRegistration ||
                          vehicles[assignModalVehicleIdx].registration ||
                          "N/A"}
                      </Text>
                      <Text
                        style={{
                      ...
                      >
                        {vehicles[assignModalVehicleIdx].type || "N/A"} •{" "}
                        {vehicles[assignModalVehicleIdx].capacity
                          ? `${vehicles[assignModalVehicleIdx].capacity} tons`
                          : "Capacity N/A"}
                      </Text>
                    </View>
                  )}
                ...
                {assigningDriver && (
                  <View style={{ alignItems: "center", marginVertical: 10 }}>
                    ...
                    <Text
                      style={{ color: colors.text.secondary, marginTop: 5 }}
                    >
                      Assigning driver...
                    </Text>
                  </View>
                ...
                <ScrollView style={{ maxHeight: 320, width: "100%" }}>
                  {drivers
                    ...
                    .map((d) => {
                      ...
                      const driverIdNumber =
                        d.idNumber || d.idDocumentNumber || "Not Available";
                      return (
                        <TouchableOpacity
                          key={d.id}
                          style={[
                        ...
                        >
                          <Image
                            ...
                            style={styles.driverAssignPhoto}
                          />
                          <View style={{ marginLeft: 8, flex: 1 }}>
                            <Text style={styles.driverAssignName}>
                              Name: {driverName}
                            </Text>
                            <Text style={styles.driverAssignName}>
                              Contact: {d.phone || "Not Available"}
                            </Text>
                            <Text style={styles.driverAssignName}>
              ...
              >
                <View style={styles.vehicleModalCard}>
                  <Text style={styles.editTitle}>
                    {vehicleEditIdx !== null ? "Edit Vehicle" : "Add Vehicle"}
                  </Text>
                  ...
                  <View style={styles.section}>
                    <Text style={styles.editLabel}>
                      Vehicle Insurance Document (PDF or Image) *
                    </Text>
                    <TouchableOpacity
                    ...
                    >
                      ...
                      <Text style={styles.uploadBtnText}>
                        {insurance ? "Change File" : "Upload Vehicle Insurance"}
                      </Text>
                    </TouchableOpacity>
                    {insurance && (
                      <Text style={styles.fileName}>
                        {insurance.fileName || insurance.uri?.split("/").pop()}
                      </Text>
                    )}
                  </View>
                  <View style={styles.editActionsRow}>
                    ...
                    <View style={styles.driverListItem} key={item.id}>
                      ...
                      >
                        ...
                        >
                          ...
                          <TouchableOpacity
                            style={styles.removeBtn}
                            onPress={() => handleRemoveDriver(index)}
                          >
                            <Ionicons
                              name="trash"
                              size={18}
              ...
              >
                <View style={styles.vehicleModalCard}>
                  ...
                  <View style={styles.section}>
                    ...
                    <TouchableOpacity
                      style={styles.uploadBtn}
                      onPress={() => pickDriverIdDoc(setRecruitIdDoc)}
                    >
                      <MaterialCommunityIcons
                        name="file-upload-outline"
                        size={22}
                      ...
                      />
                      <Text style={styles.uploadBtnText}>
                        {recruitIdDoc ? "Change File" : "Upload File"}
                      </Text>
                    </TouchableOpacity>
                    {recruitIdDoc && (
                      <Text style={styles.fileName}>
                        {recruitIdDoc.fileName ||
                          recruitIdDoc.uri?.split("/").pop()}
                      </Text>
                    )}
                  </View>
                  <View style={styles.section}>
                    ...
                    <TouchableOpacity
                      style={styles.uploadBtn}
                      onPress={() => pickDriverLicense(setRecruitLicense)}
                    >
                      <MaterialCommunityIcons
                        name="file-upload-outline"
                        size={22}
                      ...
                      />
                      <Text style={styles.uploadBtnText}>
                        {recruitLicense ? "Change File" : "Upload File"}
                      </Text>
                    </TouchableOpacity>
                    {recruitLicense && (
                      <Text style={styles.fileName}>
                        {recruitLicense.fileName ||
                          recruitLicense.uri?.split("/").pop()}
                      </Text>
                    )}
                  </View>
                  <View style={styles.editActionsRow}>
          ...
          >
            <View style={styles.modalOverlay}>
              ...
              >
                <View style={styles.vehicleModalCard}>
                  <Text style={styles.editTitle}>
                    {driverEditIdx !== null ? "Edit Driver" : "Add Driver"}
                  </Text>
                  <TouchableOpacity
                    style={{ alignSelf: "center", marginBottom: 16 }}
                    onPress={() => pickDriverPhoto(setDriverPhoto)}
                  >
                    {driverPhoto ? (
                      <Image
                  ...
                  <View style={styles.section}>
                    ...
                    <TouchableOpacity
                      style={styles.uploadBtn}
                      onPress={() => pickDriverLicense(setDriverLicense)}
                    >
                      <MaterialCommunityIcons
                        name="file-upload-outline"
                        size={22}
                      ...
                      />
                      <Text style={styles.uploadBtnText}>
                        {driverLicense ? "Change File" : "Upload File"}
                      </Text>
                    </TouchableOpacity>
                    {driverLicense && (
                      <Text style={styles.fileName}>
                        {driverLicense.fileName ||
                          driverLicense.uri?.split("/").pop()}
                      </Text>
                    )}
                  </View>
                  <View style={styles.editActionsRow}>
                    ...
                    >
                      <Text style={styles.saveText}>
                        {driverEditIdx !== null ? "Save Changes" : "Add Driver"}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </View>
    ...
    const pickIndividualProfilePhoto = async () => {
      Alert.alert(
        ...
        [
          {
            text: "Take Photo",
            onPress: () => pickIndividualProfilePhotoCamera(),
          },
          {
            text: "Choose from Gallery",
            onPress: () => pickIndividualProfilePhotoGallery(),
          },
          { text: "Cancel", style: "cancel" },
        ],
      );
        ...
        >
          ...
          >
            <View style={{ alignItems: "center", marginBottom: 8 }}>
              {individualProfilePhoto ? (
                <Image
                  source={{ uri: individualProfilePhoto.uri }}
                  style={{
                    width: 100,
                    height: 100,
                    borderRadius: 50,
          ...
          <View style={[styles.card, { marginBottom: 12 }]}>
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              {isGeneratedEmail(
                userProfile?.email || individualProfile?.email,
              ) && (
                <Text
                  style={{
                    color: colors.warning,
                ...
                onPress={async () => {
                  if (isLocationTracking) {
                    ...
                    if (!hasConsent) {
                      ...
                      console.log(
                        "⚠️ ManageTransporterScreen: No background location consent - showing disclosure modal",
                      );
                      setShowBackgroundLocationDisclosure(true);
                    } else {
                      // Consent given - safe to start tracking
                      const result =
                        await locationService.startLocationTracking(true);
                      setIsLocationTracking(result.success);
                      if (!result.success) {
                        Alert.alert(
          ...
          <View style={[styles.card, { marginTop: 8 }]}>
            ...
            {individualProfile ? (
              <View style={{ alignItems: "center", marginBottom: 8 }}>
                ...
                {individualProfile.vehicleImagesUrl &&
                individualProfile.vehicleImagesUrl.length > 0 ? (
                  <View
                    style={{
                      width: 210,
                      height: 130,
                  ...
                  >
                    <Image
                      source={{ uri: individualProfile.vehicleImagesUrl[0] }}
                      style={{ width: 210, height: 130, resizeMode: "cover" }}
                    />
                    <View
                      style={{
                        position: "absolute",
                    ...
                    >
                      ...
                      >
                        {individualProfile.vehicleRegistration}
                      </Text>
                    </View>
                  </View>
                ) : (
                ...
                >
                  {individualProfile.vehicleMake} (
                  {individualProfile.vehicleType})
                </Text>
                {/* Documents Card */}
                <View
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    >
                      <MaterialCommunityIcons
                        name={
                          individualProfile.insuranceUrl
                            ? "shield-check"
                            : "shield-alert"
                        }
                        size={20}
                      ...
                      <Text
                        style={{
                          color: individualProfile.insuranceUrl
                            ? colors.success
                            : colors.error,
                          fontWeight: "bold",
                          fontSize: 15,
                  ...
                  >
                    ...
                    >
                      <MaterialCommunityIcons
                        name={
                          individualProfile.driverLicense
                      ...
                      <Text
                        style={{
                          color: individualProfile.driverLicense
                            ? colors.success
                            : colors.error,
                          fontWeight: "bold",
                          fontSize: 15,
                  ...
                  >
                    ...
                    >
                      ...
                      <Text style={{ color: colors.primary, fontSize: 13 }}>
                        {individualProfile.vehicleCapacity
                          ? `${individualProfile.vehicleCapacity}t`
                          : "--"}
                      </Text>
                    </View>
                    {/* Body Type */}
                    ...
                    >
                      ...
                      <Text style={{ color: colors.secondary, fontSize: 13 }}>
                        {individualProfile.bodyType
                          ? individualProfile.bodyType.charAt(0).toUpperCase() +
                            individualProfile.bodyType.slice(1)
                          : "--"}
                      </Text>
                    </View>
                    {/* Drive Type */}
                    ...
                    <View
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
                        backgroundColor: individualProfile.refrigeration
                          ? "#e0f7fa"
                          : colors.text.light + "22",
                        borderRadius: 8,
                        paddingHorizontal: 10,
                        paddingVertical: 3,
                      }}
                    >
                      <MaterialCommunityIcons
                        ...
                        size={16}
                        color={
                          individualProfile.refrigeration
                            ? colors.primary
                            : colors.text.secondary
                        }
                        style={{ marginRight: 4 }}
                      ...
                      <Text
                        style={{
                          color: individualProfile.refrigeration
                            ? colors.primary
                            : colors.text.secondary,
                          fontSize: 13,
                        }}
                    ...
                    <View
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
                        backgroundColor: individualProfile.humidityControl
                          ? "#e3f2fd"
                          : colors.text.light + "22",
                        borderRadius: 8,
                        paddingHorizontal: 10,
                    ...
                    >
                      <MaterialCommunityIcons
                        ...
                        size={16}
                        color={
                          individualProfile.humidityControl
                            ? colors.primary
                            : colors.text.secondary
                        }
                        style={{ marginRight: 4 }}
                      ...
                      <Text
                        style={{
                          color: individualProfile.humidityControl
                            ? colors.primary
                            : colors.text.secondary,
                          fontSize: 13,
                        }}
                    ...
                    <View
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
                        backgroundColor: individualProfile.specialCargo
                          ? "#f3e5f5"
                          : colors.text.light + "22",
                        borderRadius: 8,
                        paddingHorizontal: 10,
                    ...
                    >
                      <MaterialCommunityIcons
                        ...
                        size={16}
                        color={
                          individualProfile.specialCargo
                            ? colors.primary
                            : colors.text.secondary
                        }
                        style={{ marginRight: 4 }}
                      ...
                      <Text
                        style={{
                          color: individualProfile.specialCargo
                            ? colors.primary
                            : colors.text.secondary,
                          fontSize: 13,
                        }}
                    ...
                    <View
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
                        backgroundColor: individualProfile.vehicleFeatures
                          ? colors.primary + "22"
                          : colors.text.light + "22",
                        borderRadius: 8,
                        paddingHorizontal: 10,
                    ...
                    >
                      <MaterialCommunityIcons
                        ...
                        size={16}
                        color={
                          individualProfile.vehicleFeatures
                            ? colors.primary
                            : colors.text.secondary
                        }
                        style={{ marginRight: 4 }}
                      ...
                      <Text
                        style={{
                          color: individualProfile.vehicleFeatures
                            ? colors.primary
                            : colors.text.secondary,
                          fontSize: 13,
                        }}
                ...
                >
                  ...
                  <TouchableOpacity
                    style={{
                      backgroundColor:
                        individualProfile.bodyType === "closed"
                          ? colors.primary
                          : colors.surface,
                      borderRadius: 8,
                      paddingVertical: 6,
                  ...
                  <TouchableOpacity
                    style={{
                      backgroundColor:
                        individualProfile.bodyType === "open"
                          ? colors.primary
                          : colors.surface,
                      borderRadius: 8,
                      paddingVertical: 6,
                ...
                >
                  <TouchableOpacity
                    onPress={() => setPhotoGalleryModalVisible(true)}
                  ...
                  <View style={styles.editActionsRow}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        Alert.alert(
                          "Vehicle Updated",
                          "Your vehicle has been replaced. Await approval if required.",
                        );
                      }}
                    >
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.editModalCard}>
              ...
              <TouchableOpacity
                style={{ alignSelf: "center", marginBottom: 16 }}
                onPress={pickIndividualProfilePhoto}
                activeOpacity={0.7}
              >
                {individualProfilePhoto ? (
                  <Image
