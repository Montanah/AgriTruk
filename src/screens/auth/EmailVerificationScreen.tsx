...
const EmailVerificationScreen = ({ navigation, route }) => {
  ...
  console.log("📧 EmailVerificationScreen received params:", {
    routeEmail,
    routePhone,
    routeRole,
    routeUserId,
  ...
  const handleVerify = async () => {
    ...
    try {
        ...
        try {
          ...
          if (currentUser) {
            ...
            if (userDoc.exists()) {
              const userData = userDoc.data();
              console.log(
                `User verification status check (attempt ${retryCount + 1}):`,
                {
              ...
              if (userData.isVerified) {
                console.log(
                  "✅ User is now verified in Firestore - proceeding with navigation",
                );
                isVerifiedInFirestore = true;
                break;
              } else {
                console.log(
                  `⏳ User not yet verified in Firestore (attempt ${retryCount + 1}/${maxRetries}) - waiting...`,
                );
                retryCount++;
      ...
      if (!isVerifiedInFirestore) {
        console.warn(
          "⚠️ User verification status not confirmed in Firestore after retries - proceeding anyway",
        );
      }
      ...
      setTimeout(async () => {
        ...
        if (role === "shipper") {
          ...
          console.log(
            "Email verification complete - signing out broker to get correct navigation stack",
          );
          ...
          if (!email || !password) {
            ...
            console.log(
              "No credentials for broker re-authentication - navigating to VerifyIdentificationDocument",
            );
            navigation.reset({
              index: 0,
              routes: [
          ...
          try {
            ...
            console.log(
              "Broker signed in again with correct role:",
            ...
            console.error(
              "Error signing in broker after verification:",
              signInError,
            );
            ...
            console.log(
              "Sign-in failed, navigating to VerifyIdentificationDocument as fallback",
            );
            navigation.reset({
              index: 0,
              routes: [
          ...
          console.log(
            "Email verification complete - navigating transporter to completion screen",
          );
          ...
          if (fromContactUpdate) {
            console.log(
              "Email verification complete from contact update - navigating back to DriverRecruitmentStatusScreen",
            );
            ...
            console.log(
              "Email verification complete - navigating job seeker to completion screen",
            );
            navigation.reset({
              index: 0,
              routes: [
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (
        ...
        errorMessage =
          "Your email is already verified. You can proceed to the app.";
      } else if (
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        <TouchableOpacity
          ...
          onPress={() =>
            handleVerificationBackNavigation(navigation, {
              email,
              phone,
              role,
              password,
