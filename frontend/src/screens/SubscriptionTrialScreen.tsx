...
const SubscriptionTrialScreen: React.FC<SubscriptionTrialScreenProps> = ({ route }) => {
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<'mpesa' | 'stripe' | null>(
    null,
  );
  ...
  const isForRenewal =
    isRenewal || subscriptionStatus?.isExpired || subscriptionStatus?.needsRenewal;

  // CRITICAL: Trials are admin-activated, not user-activated
  // Backend creates subscriptions automatically; users see status only
  const trialDuration = 90; // Standard trial duration - NEVER changed from 90 days
              ...
              routes: [
                {
                  ...
                  params: { transporterType: userType === 'company' ? 'company' : 'individual' },
                },
              ],
            });
        ...
        >
          ...
          <View style={styles.welcomeSection}>
            ...
            <Text style={styles.welcomeTitle}>
              {isForRenewal ? 'Choose Your Subscription Plan' : `Your Free Trial Status`}
            </Text>
