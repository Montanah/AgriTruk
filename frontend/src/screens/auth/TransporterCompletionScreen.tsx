...
function isTransporterProfileComplete(transporter: any) {
  ...
  // Backend stores these as: companyName, companyContact, status
  const requiredFieldMappings = [
    { frontend: "companyName", backend: ["companyName", "name"] }, // Company name
    { frontend: "companyContact", backend: ["companyContact", "contact"] }, // Company contact
  ...
  for (const fieldMapping of requiredFieldMappings) {
    let fieldValue = null;
    // Try both field names (backend uses companyName/companyContact, older might use name/contact)
    for (const fieldName of fieldMapping.backend) {
      if (transporter[fieldName]) {
        fieldValue = transporter[fieldName];
        break;
      }
    }
    if (
      ...
      console.log(
        `Missing or empty field: ${fieldMapping.frontend} (checked: ${fieldMapping.backend.join(", ")})`,
      );
      return false;
    }
  }
  ...
  const hasLogo =
    transporter.companyLogo || transporter.logo || transporter.profilePhoto;
  if (!hasLogo) {
...
export default function TransporterCompletionScreen() {
  ...
  const runProfileCheck = React.useCallback(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      setProfileCheckError(
        "Profile check timed out. Please check your connection and try again.",
      );
    }, 5000); // 5 seconds
    (async () => {
      try {
        ...
        if (
          ...
          if (isTransporterProfileComplete(data.transporter)) {
            if (data.transporter.status === "approved") {
              ...
              console.log(
                "✅ Company transporter is approved - auto-activating 90-day trial",
              );
              (async () => {
                try {
                  ...
                  if (activateResult.success) {
                    console.log(
                      "✅ Trial activated successfully for transporter, navigating to dashboard",
                    );
                    navigation.reset({
                      index: 0,
                      routes: [
                    ...
                    if (subscriptionStatus && !subscriptionLoading) {
                      if (
                        ...
                        console.log(
                          "✅ Fallback: User has active subscription/trial, navigating to dashboard",
                        );
                        navigation.reset({
                          index: 0,
                          routes: [
                        ...
                        console.log(
                          "⚠️ Fallback: No active subscription, showing dashboard",
                        );
                        navigation.reset({
                          index: 0,
                          routes: [
                      ...
                      if (
                        freshStatus.hasActiveSubscription ||
                        freshStatus.isTrialActive
                      ) {
                        navigation.reset({
                          index: 0,
                          routes: [
              ...
              ["pending", "under_review"].includes(data.transporter.status)
            ) {
              clearTimeout(timeout);
              navigation.reset({
                index: 0,
  ...
  useEffect(() => {
    const fallbackTimer = setTimeout(() => {
      if (checkingProfile && !subscriptionStatus && !subscriptionLoading) {
        console.log(
          "Subscription status timeout - proceeding with default navigation",
        );
        setCheckingProfile(false);
        ...
        try {
          ...
          if (user) {
            ...
            if (res.ok) {
              ...
              if (userData && userData.name && !companyName)
                setCompanyName(userData.name);
              if (userData && userData.phone && !companyContact)
                setCompanyContact(userData.phone);
            } else {
    ...
    Alert.alert(
      ...
      [
        { text: "Take Photo", onPress: () => handleProfilePhotoCamera() },
        {
          ...
          onPress: () => handleProfilePhotoGallery(),
        },
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        try {
          ...
          try {
            ...
            if (res.ok) {
              ...
              throw new Error(
                `FormData request failed with status: ${res.status}`,
              );
            }
          } catch (fetchError) {
          ...
          if (responseText && responseText.trim()) {
            try {
              ...
              console.log(
                "Company created successfully with FormData:",
                companyData,
              );
            } catch (parseError) {
              ...
              console.log(
                "Response is not JSON but status is OK, proceeding...",
              );
          ...
          if (isNetworkError) {
            console.log(
              "Network error detected, attempting retry with simplified request...",
            );
            ...
            try {
              ...
              if (retryRes.ok) {
                ...
                console.log(
                  "Company created successfully with retry:",
                  companyData,
                );
                ...
                if (retryRes.status === 500) {
                  console.log(
                    "500 error - company may have been created, proceeding...",
                  );
                ...
                throw new Error(
                  `Retry request failed: ${retryRes.status} - ${errorText}`,
                );
              }
            } catch (retryError) {
          ...
          if (error.name === "AbortError") {
            ...
            if (error.message.includes("Network request failed")) {
              ...
              errorMsg =
                "A company with this name or registration already exists.";
      ...
      if (e.name === "AbortError") {
        ...
        e.message &&
        e.message.includes("Unable to connect to server")
      ) {
        errorMsg =
  ...
  if (profileCheckError) {
    return (
      <View style={styles.statusCheckerContainer}>
        <Text style={[styles.statusCheckerText, { color: colors.error }]}>
          {profileCheckError}
        </Text>
        <TouchableOpacity
          style={{
  ...
  return (
    ...
    >
      ...
      <View style={styles.modernHeader}>
        ...
        <View style={styles.headerTextContainer}>
          ...
          <Text style={styles.modernHeaderSubtitle}>
            Set up your company transporter account to start managing your fleet
      ...
      >
        ...
        >
          ...
          <Text style={styles.label}>
            Company/Business Registration Number (Optional)
          </Text>
        </View>
        <View style={{ marginBottom: 8 }}>
          ...
          >
            You can skip this now. Registration will be required after 5
            completed trips.
          </Text>
        </View>
        <View
      ...
      >
        ...
        >
          As a company, you can manage your fleet, recruit drivers, and access
          qualified professionals. Your drivers will handle job bookings and
          instant requests on behalf of your company.
        </Text>
        <View
          style={{
        ...
        >
          ...
          >
            Manage your fleet, recruit drivers, and access job seeker
            marketplace.
          </Text>
        </View>
      </View>
