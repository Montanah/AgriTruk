...
export function validateMpesaPhoneNumber(phone: string): {
  ...
  if (patterns.kenyaLocal.test(cleaned)) {
    // Format: 07XXXXXXXX → +254722XXXXXX
  ...
  const operatorPatterns = {
    safaricom: /^(07|722|723|724|725|726|727|728|729)/, // Safaricom
  ...
  if (operatorInfo) {
    return {
      valid: false,
      error: `Invalid ${operatorInfo[0]} number format. Use 07XXXXXXXX or +254722XXXXXX format.`,
    };
  }

  return {
    valid: false,
    error: 'Invalid Kenyan phone number. Must be 10 digits starting with 07 or +254.',
  };
}
...
export async function processMpesaPayment(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      onStatusChange(`Processing payment (attempt ${attempt}/${maxRetries})...`);
      ...
      if (isTimeout) {
        onStatusChange(`Request timeout, retrying... (${attempt}/${maxRetries})`);
      } else {
...
export async function waitForMpesaPrompt(
  ...
  while (Date.now() - startTime < maxWaitMs) {
    try {
      ...
      // Still processing
      onStatusChange(`Waiting for M-PESA prompt... (${Math.round((Date.now() - startTime) / 1000)}s)`);
      await new Promise((resolve) => setTimeout(resolve, pollIntervalMs));
    } catch (error: any) {
  ...
  return {
    success: false,
    status: 'timeout',
    error: 'M-PESA prompt not received. Please ensure USSD is not already open and try again.',
  };
}
...
export function formatMpesaError(error?: string): string {
  const errorMap: { [key: string]: string } = {
    INVALID_PHONE: 'Please enter a valid Kenyan phone number (07XXXXXXXX or +254722XXXXXX)',
    TIMEOUT: 'Payment request timed out. Check your internet connection and try again.',
    ...
    ACCOUNT_LOCKED: 'Your M-PESA account is locked. Please contact Safaricom support.',
