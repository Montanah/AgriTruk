...
router.get(
  '/:transporterId',
  authenticateToken,
  requireRole(['transporter', 'admin', 'shipper']),
  getTransporter,
);

/**
 * @swagger
 * /api/transporters/profile/me:
 *   get:
 *     summary: Get current user's transporter profile
 *     description: Retrieve the authenticated transporter's own profile.
 *     tags: [Transporters]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: Transporter profile retrieved successfully
 *       404:
 *         description: Transporter profile not found
 *       500:
 *         description: Internal server error
 */
router.get('/profile/me', authenticateToken, requireRole('transporter'), getTransporterProfile);
 ...
 */
router.patch(
  '/:transporterId/review',
  authenticateToken,
  requireRole('admin'),
  authorize(['manage_transporters', 'super_admin']),
  reviewTransporter,
);
