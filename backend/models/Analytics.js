...
async create(date) {
  try {
    ...
    // --- Previous day (7 days earlier) ---
    const prevBase = new Date(baseDate);
    prevBase.setDate(prevBase.getDate() - 7); // go back 7 days
    const prevStart = new Date(prevBase.setHours(0, 0, 0, 0));
    const prevEnd = new Date(prevBase.setHours(23, 59, 59, 999));
    const previous = await computeMetrics(prevStart, prevEnd);
    ...
    await db.collection("analytics").doc(`day_${date}`).set(analytics);
    return analytics;
