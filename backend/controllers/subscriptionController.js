    ...
    if (action === "subscribe" || action === "update") {
      ...
      if (!existingSubscription || !existingSubscription.autoRenew) {
        return res
          ...
          .json({
            success: false,
            message: "Auto-renew not enabled or no subscription",
          });
      }
      const plan = await SubscriptionPlans.getSubscriptionPlan(
      ...
      if (subscription && subscription.transactionId === reference) {
        ...
        if (plan.price === 0) {
          ...
          const durationInDays =
            plan.duration > 12 ? plan.duration : plan.duration * 30;
          console.log(
            `✅ Payment callback: Paid plan with ${durationInDays} days (duration: ${plan.duration})`,
          );
...
exports.updateSubscriptionPlan = async (req, res) => {
  try {
    const plan = await SubscriptionPlans.updateSubscriptionPlan(
      req.params.id,
      req.body,
    );
    res
...
exports.createSubscriber = async (req, res) => {
  try {
    ...
    if (plan.price === 0) {
      ...
      const trialDays = plan.trialDays || plan.duration || 90;
      console.log(
        `🔧 Creating trial subscriber with ${trialDays} days (plan.duration: ${plan.duration}, plan.trialDays: ${plan.trialDays})`,
      );
      endDate.setDate(endDate.getDate() + trialDays);
    } else {
      ...
      const durationInDays =
        plan.duration > 12 ? plan.duration : plan.duration * 30; // Heuristic: if > 12, assume days
      console.log(
        `🔧 Creating paid subscriber with ${durationInDays} days (plan.duration: ${plan.duration})`,
      );
      endDate.setDate(endDate.getDate() + durationInDays);
    }
    const isActive = true;
    ...
    res
      ...
      .json({
        ...
        data: formatTimestamps(subscriber),
...
exports.getSubcriberStatus = async (req, res) => {
  try {
    ...
    if (
      subscriber.endDate &&
      typeof subscriber.endDate.toMillis === "function"
    ) {
      endDateMillis = subscriber.endDate.toMillis();
    } else if (subscriber.endDate && subscriber.endDate._seconds) {
      endDateMillis =
        subscriber.endDate._seconds * 1000 +
        (subscriber.endDate._nanoseconds || 0) / 1000000;
    } else if (subscriber.endDate && subscriber.endDate.seconds) {
      endDateMillis =
        subscriber.endDate.seconds * 1000 +
        (subscriber.endDate.nanoseconds || 0) / 1000000;
    } else {
    ...
    const daysRemaining = isActive
      ? Math.max(
          0,
          Math.ceil((endDateMillis - currentTime) / (1000 * 60 * 60 * 24)),
        )
      : 0;
...
async function checkTrialEligibility(userId) {
  try {
    ...
    const hasPreviousSubscriptions =
      await Subscribers.hasAnySubscription(userId);
...
exports.changePlan = async (req, res) => {
  try {
    const userId = req.user.uid;
    const { newPlanId, action, paymentMethod, phoneNumber, currency } =
      req.body;
    ...
    const currentPlan = await SubscriptionPlans.getSubscriptionPlan(
      currentSubscriber.planId,
    );
...
function calculateProratedAmount(currentSubscriber, currentPlan, newPlan) {
  ...
  const endDateMillis = currentSubscriber.endDate.toMillis
    ...
    : currentSubscriber.endDate._seconds * 1000 +
      currentSubscriber.endDate._nanoseconds / 1000000;
...
async function cancelSubscription(
  ...
  const updatedSubscription = await Subscribers.update(
    subscriber.id,
    updateData,
  );
  ...
  if (refundAmount > 0) {
    await processRefund(
      subscriber.payerId,
      refundAmount,
      `Refund for cancelled subscription: ${currentPlan.name}`,
    );
  }
...
function calculateRefundAmount(subscriber, plan) {
  ...
  const startDateMillis = convertFirestoreTimestampToMillis(
  ...
  try {
    ...
    if (planId) {
      ...
      if (!trialPlan || trialPlan.price !== 0) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid trial plan" });
      }
    } else {
      trialPlan = await SubscriptionPlans.getTrialPlan();
    }
    ...
    if (!trialPlan) {
      return res
        .status(404)
        .json({ success: false, message: "No trial plan found" });
    }
