...
const generateReadableId = (
  ...
  const suffix = computeShortToken(
    seed || `${date.getTime()}-${Math.random()}`,
  );
...
exports.createBooking = async (req, res) => {
  try {
    ...
    if (!["Agri", "Cargo"].includes(bookingType)) {
      return res
        .status(400)
        .json({ message: 'Invalid booking type. Use "Agri" or "Cargo"' });
    }
    ...
    if (!["instant", "booking"].includes(bookingMode)) {
      return res
        .status(400)
        .json({ message: 'Invalid booking mode. Use "instant" or "booking"' });
    }
    ...
    const validateAndGeocodeLocation = async (location, fieldName) => {
      ...
      if (
        ...
        throw new Error(
          `${fieldName}.address is required and must be a non-empty string`,
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Could not geocode ${fieldName} address: ${location.address}`,
        );
      }
    };
    ...
    try {
      ...
      const geocodedFromLocation = await validateAndGeocodeLocation(
        fromLocation,
        "fromLocation",
      );
      const geocodedToLocation = await validateAndGeocodeLocation(
        toLocation,
        "toLocation",
      );
    ...
    if (bookingMode === "booking") {
      if (!pickUpDate) {
        return res
          .status(400)
          .json({ message: "pickUpDate is required for booking type" });
      }
    ...
    if (recurrence && recurrence.isRecurring) {
      ...
      if (!frequency || !timeFrame || !duration) {
        return res
          ...
          .json({
            message:
              "Frequency, timeFrame, and duration are required for recurring requests",
          });
      }
      recurrenceData = {
        isRecurring: true,
        ...
        timeFrame,
        duration,
        startDate: validatedPickUpDate
          ? Timestamp.fromDate(validatedPickUpDate)
          : Timestamp.now(),
    ...
    if (bookingType === "Cargo" && specialCargo.length > 0) {
      const invalidItems = specialCargo.filter(
        (item) => !validSpecialCargo.includes(item),
      );
      if (invalidItems.length > 0) {
        return res
          .status(400)
          .json({
            message: `Invalid special cargo types: ${invalidItems.join(", ")}`,
          });
      }
    }
    ...
    const {
      ...
      paymentBreakdown,
      estimatedCostRange,
    } = calculateTransportCost(bookingDataForCost);
    ...
    const bookingData = {
      ...
      clientId:
        (req.body.brokerData && req.body.brokerData.clientId) ||
        req.body.clientId ||
        null,
      weightKg,
      ...
      nightSurcharge: !!nightSurcharge,
      statusHistory: [
        { status: "pending", timestamp: Timestamp.now(), reason: null },
      ],
    };
    ...
    const createdAtDate = booking.createdAt?.toDate
      ...
      : new Date();
    const computedReadableId = generateReadableId(
      bookingType,
      bookingMode,
      !!consolidated,
      createdAtDate,
    ...
    if (!booking.readableId || booking.readableId !== computedReadableId) {
      try {
        await Booking.update(booking.bookingId, {
          readableId: computedReadableId,
        });
    ...
    if (bookingMode === "instant") {
      const matchedTransporter = await MatchingService.matchBooking(
        booking.bookingId,
      );
...
function calculateEndDate(startDate, duration) {
  ...
  switch (unit.toLowerCase()) {
    ...
    case "years":
      return new Date(
        startDate.getFullYear() + numValue,
        startDate.getMonth(),
        startDate.getDate(),
      );
    default:
...
exports.getBrokerScopedBookings = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        success: false,
        message: "Failed to fetch broker scoped bookings",
      });
  }
};
...
exports.updateBooking = async (req, res) => {
  try {
    ...
    if (updates.cancelledAt && typeof updates.cancelledAt === "string") {
      updates.cancelledAt = admin.firestore.Timestamp.fromDate(
        new Date(updates.cancelledAt),
      );
    }
    if (updates.startedAt && typeof updates.startedAt === "string") {
      updates.startedAt = admin.firestore.Timestamp.fromDate(
        new Date(updates.startedAt),
      );
    }
    if (updates.acceptedAt && typeof updates.acceptedAt === "string") {
      updates.acceptedAt = admin.firestore.Timestamp.fromDate(
        new Date(updates.acceptedAt),
      );
    }
    if (updates.completedAt && typeof updates.completedAt === "string") {
      updates.completedAt = admin.firestore.Timestamp.fromDate(
    ...
    if (updates.status === "pending") {
      ...
      updates = {
        ...
        ...(preservedFields.cancellationReason !== undefined && {
          cancellationReason: preservedFields.cancellationReason,
        }),
        ...(preservedFields.cancelledAt !== undefined && {
          cancelledAt: preservedFields.cancelledAt,
        }),
      };
    }
    ...
    // If status is being set to 'completed', check registration requirement
    if (updates.status === "completed") {
      // Get booking to find company ID
      const booking = await Booking.get(bookingId);
      if (booking && booking.companyId) {
        // Enforce registration requirement after 5 trips
        const companyController = require("./companyController");
        await companyController.enforceRegistrationAfterTrip(
          bookingId,
          booking.companyId,
        );
      }
    ...
    res
      ...
      .json({
        ...
        booking: formatTimestamps(updatedBooking),
      });
  } catch (err) {
      ...
      bookings.map(async (booking) => {
        ...
        if (booking.userId) {
          try {
            ...
            console.error(
              "Error fetching client for booking:",
              booking.id,
              error,
            );
            enrichedBooking.client = {
        ...
        if (booking.vehicleId) {
          try {
            ...
            if (transporter && transporter.companyId) {
              ...
              const vehicle = await Vehicle.get(
            ...
            console.error(
              "Error fetching vehicle for booking:",
              booking.id,
              error,
            );
            enrichedBooking.vehicle = {
...
exports.getTransporterRouteLoads = async (req, res) => {
  try {
    ...
    if (!transporter.vehicleCapacity) {
      return res
        .status(404)
        .json({ message: "Transporter has no vehicle capacity" });
    }
    ...
    for (const doc of bookingsSnapshot) {
      ...
      if (transporter.currentRoute && transporter.currentRoute.length > 0) {
        const lastKnownLocation =
          ...
          transporter.currentRoute[transporter.currentRoute.length - 1]
            .location;
...
exports.getFleetStatus = async (req, res) => {
  try {
    ...
    const fleetStatus = transporters.map((transporter) => {
      const booking = transporter.transporterId
        ? bookingMap[transporter.transporterId]
        : null;
      ...
      return {
        ...
        driver: {
          ...
          name: user
            ? transporter.displayName || `${user.name}`
            : "Unknown Driver",
    ...
    if (status) {
      filteredFleet = fleetStatus.filter(
        (vehicle) => vehicle.status.toLowerCase() === status.toLowerCase(),
      );
    }
...
exports.hasExpiredDocuments = (transporter) => {
  ...
  if (
    transporter.insuranceExpiryDate &&
  ...
  if (
    transporter.driverLicenseExpiryDate &&
  ...
  // Check ID expiry
  if (transporter.idExpiryDate && transporter.idExpiryDate.toDate() < now) {
    return true;
  }
...
exports.acceptBooking = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        "❌ Transporter/Driver not found, continuing without details:",
        error.message,
      );
    }
    ...
    if (vehicleId) {
      try {
        ...
        if (isCompanyDriver) {
          ...
          console.log(
            "✅ Company driver detected, getting vehicle from company collection",
          );
          ...
          console.log(
            "✅ Individual transporter detected, vehicle data should be in transporter document",
          );
        ...
        console.log(
          "Error determining transporter type, continuing without vehicle details:",
          error.message,
        );
      }
    }
    ...
    const updates = {
      ...
      transporterExperience: transporter?.totalTrips
        ? `${transporter.totalTrips} trips`
        : "New transporter",
      transporterAvailability: transporter?.acceptingBooking
        ? "Available"
        : "Offline",
      ...
      vehicleRegistration:
        ...
        vehicle?.reg ||
        transporter?.vehicleRegistration ||
        null,
      vehicleColor:
        vehicle?.vehicleColor ||
        vehicle?.color ||
        transporter?.vehicleColor ||
        null,
      vehicleCapacity:
...
exports.getBookingStatus = async (req, res) => {
  try {
    ...
    const hasAccess =
      booking.userId === userId || booking.transporterId === userId;
    if (!hasAccess) {
      return res.status(403).json({
        success: false,
...
exports.getDriverRouteLoads = async (req, res) => {
  try {
    ...
    for (const booking of bookingsSnapshot) {
      ...
      const isCapacityCompatible =
        (!booking.weightKg ||
          !driver.assignedVehicleDetails?.capacityKg ||
          booking.weightKg <= driver.assignedVehicleDetails.capacityKg) &&
        (!booking.needsRefrigeration ||
          driver.assignedVehicleDetails?.refrigerated) &&
        (!booking.humidityControl ||
          driver.assignedVehicleDetails?.humidityControl);
    ...
    const enrichedBookings = await Promise.all(
      acceptedBookings.map(async (booking) => {
        ...
        if (!enriched.readableId && enriched.bookingId) {
          ...
          const bookingDate = enriched.createdAt?.toDate
            ? enriched.createdAt.toDate()
            : enriched.createdAt
              ? new Date(enriched.createdAt)
              : null;
          if (bookingDate) {
        ...
        if (enriched.costBreakdown) {
          enriched.pricing = {
            ...
            perishableSurcharge:
              enriched.costBreakdown.perishableSurcharge || 0,
            refrigerationSurcharge:
              enriched.costBreakdown.refrigerationSurcharge || 0,
  ...
  try {
    ...
    if (
      ...
      if (!success) {
        ...
        const fallbackDurationMinutes = Math.round(
          (fallbackDistance / 60) * 60,
        ); // Assume 60 km/h average
      ...
      const { cost: baseCost, costBreakdown } =
        calculateTransportCost(bookingDataForCost);
      ...
      return res.status(400).json({
        success: false,
        message:
          "Location coordinates (latitude, longitude) are required for accurate estimates",
      });
    }
  } catch (error) {
...
exports.cancelBooking = async (req, res) => {
  try {
    ...
    if (booking.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Booking is not available for cancellation" });
    }
