...
if (!auth || !db) {
  console.error(
    "CRITICAL: Firebase not properly initialized. Auth:",
    !!auth,
    "Firestore:",
    !!db,
  );
  ...
  try {
    ...
    try {
      ...
      const checkResponse = await fetch(
        `${API_ENDPOINTS.DRIVERS}/check/${userId}`,
        {
          method: "GET",
          headers: {
      ...
      console.log(
        "App.tsx: Driver check endpoint response status:",
        checkResponse.status,
      );
      if (checkResponse.ok) {
        const checkData = await checkResponse.json();
        console.log(
          "App.tsx: Driver check API response:",
          JSON.stringify(checkData, null, 2),
        );

        if (checkData.success && checkData.isDriver && checkData.driver) {
          console.log(
            "App.tsx: Found company driver via drivers/check endpoint",
          );
          return {
            isDriver: true,
            driverType: "company",
        ...
        try {
          ...
          console.log(
            "App.tsx: Driver check endpoint error response:",
            JSON.stringify(errorData, null, 2),
          );
        } catch (e) {
          console.log(
            "App.tsx: Driver check endpoint failed with status:",
            checkResponse.status,
          );
        }
      }
      ...
      const companyResponse = await fetch(
        `${API_ENDPOINTS.COMPANIES}/driver/${userId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.log(
        "App.tsx: Company driver check response status:",
        companyResponse.status,
      );
      if (companyResponse.ok) {
        const companyData = await companyResponse.json();
        console.log(
          "App.tsx: Company driver API response:",
          JSON.stringify(companyData, null, 2),
        );

        if (companyData.success && companyData.driver) {
          console.log(
            "App.tsx: Found company driver via companies/driver endpoint",
          );
          return {
            isDriver: true,
            driverType: "company",
            driverData: companyData.driver,
          };
        }
      }
    ...
    try {
      const transporterResponse = await fetch(
        `${API_ENDPOINTS.TRANSPORTERS}/profile`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    console.warn(
      "Could not verify driver status (network error):",
      error.message,
    );
    return false;
  }
...
const checkCompanyProfileComplete = (companyData: any) => {
  ...
  for (const field of companyRequiredFields) {
    if (
      !companyData[field] ||
      (typeof companyData[field] === "string" &&
        companyData[field].trim() === "") ||
...
const checkTransporterProfileComplete = (transporterData: any) => {
  ...
  for (const field of requiredFields) {
    if (
      !transporterData[field] ||
      (typeof transporterData[field] === "string" &&
        transporterData[field].trim() === "") ||
      (Array.isArray(transporterData[field]) &&
        transporterData[field].length === 0)
    ) {
      return false;
    }
  }
  ...
  if (transporterType === "individual") {
    if (
      !Array.isArray(transporterData.vehicleImagesUrl) ||
      transporterData.vehicleImagesUrl.length === 0
    ) {
      return false;
    }
  }
  ...
  // Status must be at least 'pending', 'under_review', or 'approved'
  if (
    !["pending", "under_review", "approved"].includes(transporterData.status)
  ) {
    return false;
  }
...
const checkSubscriptionStatus = async (
  ...
  try {
    ...
    const subscriptionService =
      require("./src/services/subscriptionService").default;
...
export default function App() {
  ...
  const [
    showGlobalBackgroundLocationDisclosure,
    setShowGlobalBackgroundLocationDisclosure,
  ...
  React.useEffect(() => {
    ...
    if (!auth || !db) {
      console.error(
        "Firebase not properly initialized - Auth:",
        !!auth,
        "Firestore:",
        !!db,
      );
        ...
        if (firebaseUser) {
          try {
            ...
            try {
              ...
              console.error(
                "App.tsx: Error fetching from Firestore:",
                firestoreError,
              );
            ...
            if (data) {
              ...
              try {
                const {
                  NotificationHelper,
                } = require("./src/services/notificationHelper");
              ...
              if (needsBackgroundLocation && !hasCheckedGlobalConsent) {
                ...
                if (shouldShowDisclosure) {
                  ...
                  if (data.role === "driver") {
                    ...
                    (data.role === "transporter" && !data.transporterType)
                  ) {
                    setDisclosureUserRole("company");
                    setDisclosureTransporterType("company");
                    console.log(
                      "📢 App.tsx: Setting disclosure for company transporter",
                    );
                  } else {
                    setDisclosureUserRole("individual");
                    setDisclosureTransporterType("individual");
                    console.log(
                      "📢 App.tsx: Setting disclosure for individual transporter",
                    );
                  }
                  ...
                  console.log(
                    "📢 App.tsx: Global background location disclosure modal will be shown - navigation blocked until user responds",
                  );
                } else {
                  console.log(
              ...
              if (data.role === "transporter") {
                ...
                try {
                  ...
                  console.warn(
                    "App.tsx: Error checking driver status:",
                    driverCheckError,
                  );
                ...
                try {
                  ...
                  try {
                    ...
                    console.warn(
                      "App.tsx: Error getting auth token:",
                      tokenError,
                    );
                  ...
                  try {
                    companyResponse = await fetch(
                      `${API_ENDPOINTS.COMPANIES}/transporter/${firebaseUser.uid}`,
                      {
                        headers: {
                          Authorization: `Bearer ${token}`,
                          "Content-Type": "application/json",
                    ...
                    console.warn(
                      "App.tsx: Error fetching company data:",
                      fetchError,
                    );
                    // Set defaults and let outer catch handle the rest
                    setProfileCompleted(false);
                  ...
                  if (companyResponse.ok) {
                    ...
                    try {
                      ...
                      console.warn(
                        "App.tsx: Error parsing company response:",
                        jsonError,
                      );
                      // Set defaults and let outer catch handle the rest
                      setProfileCompleted(false);
                    ...
                    if (companyData && companyData.length > 0) {
                      ...
                      const isProfileComplete =
                        checkCompanyProfileComplete(company);
                      ...
                      try {
                        ...
                        console.warn(
                          "App.tsx: Error checking subscription status:",
                          subError,
                        );
                  ...
                  console.error(
                    "App.tsx: Error checking transporter profile:",
                    e,
                  );
                ...
                if (data.isVerified) {
                  try {
                    ...
                    console.warn(
                      "App.tsx: Error checking broker subscription status:",
                      subError,
                    );
                    // Set default subscription status on error
                    setSubscriptionStatus({
                ...
                console.log(
                  "App.tsx: Business user detected - no subscription needed",
                );
                ...
                console.log(
                  "App.tsx: Job seeker detected - skipping subscription checks",
                );
                ...
                try {
                  ...
                  if (!firebaseUser) {
                    console.log(
                      "App.tsx: No Firebase user found for job seeker profile check",
                    );
                    setProfileCompleted(false);
                    return;
                  }
                  ...
                  const jobSeekerResponse = await fetch(
                    `${API_ENDPOINTS.JOB_SEEKERS}/user/${firebaseUser.uid}`,
                    {
                  ...
                  if (jobSeekerResponse.ok) {
                    ...
                    console.log(
                      "App.tsx: Raw job seeker API response:",
                      jobSeekerData,
                    );
                    ...
                    const vehicleClasses =
                      application?.experience?.vehicleClassesExperience ||
                      application?.vehicleClassesExperience ||
                      application?.vehicleClasses ||
                      [];
                    const specializations =
                      application?.experience?.specializations ||
                    ...
                    console.log(
                      "App.tsx: Job seeker profile completeness check:",
                      {
                        hasProfilePhoto:
                          application?.profilePhoto &&
                          application.profilePhoto.trim() !== "",
                    ...
                    console.log(
                      "App.tsx: Job seeker profile not found - setting incomplete",
                    );
                    setProfileCompleted(false);
                  }
                } catch (error) {
                  console.log(
                    "App.tsx: Error checking job seeker profile:",
                    error,
                  );
                  setProfileCompleted(false);
                }
                ...
                if (
                  !isDriver ||
                  typeof isDriver !== "object" ||
                  !(isDriver as any)?.isDriver
                ) {
                ...
                console.log(
                  "App.tsx: ========== DRIVER ROLE DETECTED ==========",
                );
                console.log(
                  "App.tsx: Checking driver via Firestore fields first...",
                );

                // Check Firestore user document for companyId/driverId fields (most reliable)
                if (data.companyId || data.driverId) {
                  console.log(
                    "App.tsx: ✓ Company driver detected via Firestore user document",
                  );
                  console.log(
                    "App.tsx: companyId:",
                    data.companyId,
                  ...
                  console.log(
                    "App.tsx: ✓ isDriver state set to company driver",
                  );
                } else {
                  console.log(
                    "App.tsx: No companyId/driverId in Firestore - checking via API...",
                  );
                  ...
                  try {
                    ...
                    if (firebaseUser) {
                      console.log(
                        "App.tsx: Calling checkIfDriver with userId:",
                        firebaseUser.uid,
                      );
                      console.log(
                        "App.tsx: API endpoint:",
                        `${API_ENDPOINTS.DRIVERS}/check/${firebaseUser.uid}`,
                      );
                      ...
                      console.log(
                        "App.tsx: Driver check result type:",
                        typeof driverCheck,
                      );
                      console.log(
                        "App.tsx: Driver check result:",
                        JSON.stringify(driverCheck, null, 2),
                      );

                      if (
                        driverCheck &&
                        typeof driverCheck === "object" &&
                        driverCheck.isDriver &&
                        driverCheck.driverType === "company"
                      ) {
                        console.log(
                          "App.tsx: ✓✓✓ COMPANY DRIVER CONFIRMED VIA API ✓✓✓",
                        );
                        setIsDriver(driverCheck);
                        ...
                        console.log(
                          "App.tsx: ✗ Not a company driver - treating as job seeker",
                        );
                        setIsDriver(false);
                        setProfileCompleted(false);
                      }
                      ...
                      console.log(
                        "App.tsx: ✗ No Firebase user for driver check",
                      );
                      setIsDriver(false);
                      setProfileCompleted(false);
                    }
              ...
              console.log(
                "App.tsx: User not found in Firestore - signing out and redirecting to welcome",
              );
              try {
                await auth.signOut();
            ...
            if (firebaseUser.emailVerified) {
              console.log(
                "App.tsx: Firestore failed but Firebase Auth shows email verified - using as fallback",
              );
              setIsVerified(true);
              ...
              console.log(
                "App.tsx: Firestore failed and user not verified - showing error state",
              );
              setIsVerified(false);
              setRole(null);
              setProfileCompleted(false);
  ...
  if (loading) {
    ...
    return (
      <ErrorBoundary>
        <>
          <NavigationContainer
            key={`${user ? user.uid : "guest"}-${role || "none"}`}
          >
          ...
          <BackgroundLocationDisclosureModal
            ...
            onDecline={async () => {
              try {
                ...
                console.warn(
                  "App.tsx: Error saving background location decline:",
                  error,
                );
              }
              setShowGlobalBackgroundLocationDisclosure(false);
  ...
  if (role === "job_seeker") {
    console.log(
      "App.tsx: CRITICAL - Job seeker detected, ensuring proper routing to job seeker screens only",
    );
  }
  ...
  if (role === "driver") {
    console.log(
      "App.tsx: Driver role detected in routing logic - isDriver:",
      isDriver,
    );
  }
  ...
  if (!user) {
    ...
    screens = (
      <>
        ...
        <Stack.Screen
          ...
          component={require("./src/screens/auth/PasswordResetScreen").default}
        />
        <Stack.Screen
          name="ChangePasswordScreen"
          component={require("./src/screens/auth/ChangePasswordScreen").default}
        />
        <Stack.Screen
          name="AccountDeleted"
          component={require("./src/screens/AccountDeletedScreen").default}
        />
        <Stack.Screen
          name="RestoreAccount"
          component={require("./src/screens/RestoreAccountScreen").default}
        />
        <Stack.Screen
        ...
        <Stack.Screen
          ...
          component={
            require("./src/screens/VerifyIdentificationDocumentScreen").default
          }
          options={{
        ...
        <Stack.Screen
          name="BookingList"
          component={require("./src/screens/BookingListScreen").default}
        />
        <Stack.Screen
        ...
        <Stack.Screen
          ...
          component={RatingSubmissionScreen}
        />
        <Stack.Screen
          name="RatingAnalytics"
          component={RatingAnalyticsScreen}
        />
    ...
    console.log(
      "App.tsx: Verified user but no role data - routing to role selection",
    );
    ...
    initialRouteName =
      preferredMethod === "phone" ? "PhoneOTPScreen" : "EmailVerification";
    screens = (
      <>
        <Stack.Screen name="Welcome" component={WelcomeScreen} />
        <Stack.Screen
        ...
        />
        <Stack.Screen
          name="DriverRecruitmentStatusScreen"
          component={
            require("./src/screens/DriverRecruitmentStatusScreen").default
          }
        />
      </>
    );
    ...
    if (role === "job_seeker") {
      console.log(
        "App.tsx: Job seeker detected in verified section - ensuring proper routing",
      );
    }
    ...
    if (role === "shipper") {
      ...
      screens = (
        <>
          ...
          <Stack.Screen
            ...
            component={require("./src/screens/TripDetailsScreen").default}
          />
          <Stack.Screen
            name="TrackingScreen"
            component={require("./src/screens/TrackingScreen").default}
          />
      ...
      console.log(
        "App.tsx: Verified broker detected - checking verification and subscription status",
      );
      ...
      const hasActiveSub =
        ...
        subscriptionStatus?.isTrialActive === true;

      if (isExpired) {
        // Broker has expired subscription - go to expired screen for renewal/upgrade
        console.log(
          "App.tsx: Broker subscription expired - routing to expired screen",
        );
        ...
        console.log(
          "App.tsx: Broker has active subscription/trial - routing to dashboard",
        );
      ...
      if (!profileCompleted) {
        ...
        console.log(
          "App.tsx: Profile not completed - routing to completion screen",
        );
        ...
        screens = (
          <>
            ...
            />
            <Stack.Screen
              name="JobManagementScreen"
              component={JobManagementScreen}
            />
            <Stack.Screen
              name="RouteLoadsScreen"
              component={RouteLoadsScreen}
            />
            <Stack.Screen
              name="BookingConfirmation"
              component={BookingConfirmationScreen}
        ...
        console.log(
          "App.tsx: Profile completed but not approved - routing to processing screen",
        );
        ...
        console.log(
          "App.tsx: Profile completed and approved - checking subscription status",
        );
        ...
        if (isApiError) {
          ...
          console.log(
            "App.tsx: Subscription API error - routing to trial activation for safety",
          );
          ...
          screens = (
            <>
              <Stack.Screen
                ...
                initialParams={{
                  userType:
                    userData?.transporterType === "company"
                      ? "company"
                      : "individual",
          ...
          console.log(
            "App.tsx: Subscription expired - routing to expired screen",
          );
          initialRouteName = "SubscriptionExpired";
          screens = (
            <>
              <Stack.Screen
                name="SubscriptionExpired"
                component={
                  require("./src/screens/SubscriptionExpiredScreen").default
                }
                initialParams={{
                  userType:
                    userData?.transporterType === "company"
          ...
          // No active subscription - Auto-activate trial for approved company transporters
          console.log(
            "App.tsx: No active subscription detected - auto-activating trial for approved transporter",
          );
          ...
          (async () => {
            try {
              console.log(
                "🔄 Auto-activating 90-day trial for approved transporter...",
              );
              const activateResult =
                await subscriptionService.activateTrial("transporter");

              if (activateResult.success) {
                console.log(
                  "✅ Trial activated successfully - user can now access dashboard",
                );
              } else {
                console.warn(
                  "⚠️ Trial activation failed, but allowing dashboard access:",
                  activateResult.message,
                );
              }
            } catch (error) {
          ...
          console.log(
            "App.tsx: Has active subscription - routing to dashboard",
          );
          ...
          if (isDriver && typeof isDriver === "object" && isDriver.isDriver) {
            console.log(
              "App.tsx: Driver detected - routing to driver dashboard",
            );
          ...
          screens = (
            <>
              ...
              <Stack.Screen
                ...
                component={
                  require("./src/screens/TransporterHomeScreen").default
                }
              />
              <Stack.Screen
                name="ServiceRequest"
              ...
              <Stack.Screen
                name="RequestForm"
                component={
                  require("./src/components/common/RequestForm").default
                }
              />
              <Stack.Screen name="MainTabs" component={MainTabNavigator} />
              <Stack.Screen
              ...
              <Stack.Screen
                name="MapViewScreen"
                component={require("./src/screens/MapViewScreen").default}
              />
              <Stack.Screen
                name="TransporterBookingManagement"
                component={
                  require("./src/screens/TransporterBookingManagementScreen")
                    .default
                }
              />
              <Stack.Screen
              ...
              <Stack.Screen
                ...
                component={
                  require("./src/screens/SubscriptionManagementScreen").default
                }
              />
              <Stack.Screen
                name="ContactCustomer"
                component={
                  require("./src/screens/ContactCustomerScreen").default
                }
              />
              <Stack.Screen
                name="EmailVerification"
      ...
      console.log(
        "App.tsx: Driver role in routing - checking isDriver state:",
        JSON.stringify(isDriver),
      );
      ...
      if (
        ...
        typeof isDriver === "object" &&
        (isDriver as any)?.isDriver &&
        (isDriver as any)?.driverType === "company"
      ) {
        // Company-recruited driver - route to driver tabs
        console.log(
          "App.tsx: Company driver detected via state - routing to driver dashboard",
        );
        initialRouteName = "DriverTabs";
        screens = (
          <>
        ...
        console.log(
          "App.tsx: Driver role but isDriver state indicates not a company driver - will fall through to job seeker logic",
        );
      }
    } else if (
      role === "job_seeker" ||
      ...
      console.log(
        "App.tsx: Job seeker detected - routing to recruitment status screen only",
      );

      // Check if job seeker has completed their profile
      if (!profileCompleted) {
        console.log(
          "App.tsx: Job seeker profile incomplete - routing to job seeker completion screen",
        );
        ...
        console.log(
          "App.tsx: Job seeker profile complete - routing to recruitment status",
        );
      ...
      if (role === "job_seeker") {
        console.error(
          "App.tsx: ERROR - Job seeker should not reach fallback routing!",
        );
    ...
    if (role === "transporter") {
      ...
      console.log(
        "App.tsx: Unverified transporter detected - checking Firebase Auth email verification status",
      );

      // Check Firebase Auth email verification status as fallback
      if (user.emailVerified) {
        console.log(
          "App.tsx: Firebase Auth shows email verified - routing to TransporterCompletionScreen",
        );
        initialRouteName = "TransporterCompletionScreen";
        screens = (
          <>
        ...
        const preferredMethod =
          userData?.preferredVerificationMethod || "phone";
        initialRouteName =
          preferredMethod === "phone" ? "PhoneOTPScreen" : "EmailVerification";
        screens = (
          <>
      ...
      console.log(
        "App.tsx: Unverified job seeker detected - routing to verification",
      );
      const preferredMethod = userData?.preferredVerificationMethod || "phone";
      initialRouteName =
        preferredMethod === "phone" ? "PhoneOTPScreen" : "EmailVerification";
    ...
    if (!isVerified) {
      ...
      screens = (
        <>
          ...
          />
          <Stack.Screen
            name="BrokerHomeScreen"
            component={require("./src/screens/BrokerHomeScreen").default}
          />
          <Stack.Screen
            name="BrokerManagementScreen"
            component={require("./src/screens/BrokerManagementScreen").default}
          />
          <Stack.Screen
            name="BrokerRequestScreen"
            component={require("./src/screens/BrokerRequestScreen").default}
          />
          <Stack.Screen
            name="BrokerNetworkScreen"
            component={require("./src/screens/BrokerNetworkScreen").default}
          />
          <Stack.Screen
            name="BrokerRequestsScreen"
            component={require("./src/screens/BrokerRequestsScreen").default}
          />
          <Stack.Screen
            name="TripDetailsScreen"
            component={TripDetailsScreen}
      ...
      if (isExpired) {
        ...
        screens = (
          <>
            ...
            <Stack.Screen
              name="BrokerManagementScreen"
              component={
                require("./src/screens/BrokerManagementScreen").default
              }
            />
            <Stack.Screen
              name="BrokerRequestScreen"
            ...
            <Stack.Screen
              name="RequestForm"
              component={require("./src/components/common/RequestForm").default}
            />
            <Stack.Screen
              name="TripDetailsScreen"
              component={TripDetailsScreen}
        ...
        screens = (
          <>
            ...
            <Stack.Screen
              name="ContactCustomer"
              component={require("./src/screens/ContactCustomerScreen").default}
            />
          </>
        );
      }
    ...
    if (!profileCompleted) {
      ...
      screens = (
        <>
          ...
          <Stack.Screen
            name="TransporterHome"
            component={require("./src/screens/TransporterHomeScreen").default}
          />
          <Stack.Screen
            name="ServiceRequest"
            component={ServiceRequestScreen}
          ...
          />
          <Stack.Screen
            name="CompanyFleetPlans"
            component={CompanyFleetPlansScreen}
          />
          <Stack.Screen
            name="PaymentScreen"
            component={require("./src/screens/PaymentScreen").default}
      ...
      screens = (
        <>
          <Stack.Screen
            ...
            component={TransporterProcessingScreen}
            initialParams={{
              transporterType: userData?.transporterType || "company",
            }}
          />
          <Stack.Screen
            name="SubscriptionTrial"
    ...
    console.log(
      "App.tsx: Business user routing - isVerified:",
      isVerified,
      "userData:",
      userData,
    );
    ...
    screens = (
      <>
        ...
        <Stack.Screen
          name="EmailVerification"
          component={
            require("./src/screens/auth/EmailVerificationScreen").default
          }
        />
  ...
  if (showGlobalBackgroundLocationDisclosure && disclosureUserRole) {
    console.log(
      "📢 App.tsx: ========== SHOWING GLOBAL BACKGROUND LOCATION DISCLOSURE ==========",
    );
    console.log(
      "📢 App.tsx: This is the Prominent Disclosure required by Google Play Store",
    );
    console.log(
      "📢 App.tsx: Navigation is BLOCKED until user responds to this disclosure",
    );
    console.log(
      "📢 App.tsx: User role:",
      disclosureUserRole,
      "Transporter type:",
      disclosureTransporterType,
    );
        ...
        <BackgroundLocationDisclosureModal
          ...
          onAccept={async () => {
            try {
              ...
              console.error(
                "App.tsx: Error handling disclosure acceptance:",
                error,
              );
          ...
          onDecline={async () => {
            try {
              console.log(
                "❌ App.tsx: User declined global background location disclosure",
              );
              console.log(
                "❌ App.tsx: Saving declined status - navigation will proceed but background location disabled",
              );
              await locationService.saveBackgroundLocationConsent(false);
              setShowGlobalBackgroundLocationDisclosure(false);
              willShowDisclosureRef.current = false; // Reset ref
              ...
              setLoading(false);
              console.log(
                "❌ App.tsx: Declined status saved - navigation will now proceed",
              );
            } catch (error: any) {
              console.error(
                "App.tsx: Error handling disclosure decline:",
                error,
              );
              // Even on error, allow navigation to proceed
              setShowGlobalBackgroundLocationDisclosure(false);
